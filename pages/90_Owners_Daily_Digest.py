...
# Optional imports – page is resilient if these modules are missing
try:
...
if not HAS_STORE:
    st.warning("Storage helper not found. Drop in `src/components/digest_store.py` from the delta zip.")
else:
    pnl = get_pnl()
    health = get_health()
    ...
    with c5:
        vs = health.get("vagal_score", 0)
        st.metric("Vagal Score", f"{vs}/100")
    ...
    st.caption("Tip: These numbers auto-fill from JSON files under `data/vega/`. See README in delta zip.")
    ...
    for idx, t in enumerate(tasks_state["tasks"]):
        ...
        # update "done" if toggled
        if done != t.get("done", False):
            t["done"] = done

    if to_remove or st.session_state.get("_tasks_dirty_", False):
        # Remove tasks marked for deletion and persist
        for i in sorted(to_remove, reverse=True):
            tasks_state["tasks"].pop(i)
        set_tasks(tasks_state)
        # Reset dirty flag
        st.session_state["_tasks_dirty_"] = False

    st.divider()

    st.subheader("Owner Notes")
    notes_val = get_notes()
    notes = st.text_area("Daily Notes", value=notes_val, height=200, placeholder="Key decisions, alerts, and follow-ups…")
...
# ──────────────────────────────────────────────────────────────────────────────
# Optional: Today's Trades Queue
# ──────────────────────────────────────────────────────────────────────────────
st.divider()
st.subheader("📌 Today's Trades Queue")
if 'HAS_QUEUE' in globals() and HAS_QUEUE:
    render_queue()
else:
    st.caption("today_queue component not available. If you have it, ensure import path is `src/components/today_queue.py`.")
