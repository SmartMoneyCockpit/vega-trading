...
def tiles_from_files(files: list, bench_df: pd.DataFrame = None, price_col='close'):
    ...
    for f in files:
        try:
            ...
            import os as _os
            sym = _os.path.splitext(_os.path.basename(name))[0].upper()
...
def save_snapshot(df: pd.DataFrame, region: str, export_dir='data/exports'):
    Path(export_dir).mkdir(parents=True, exist_ok=True)
    day = datetime.now().strftime('%Y%m%d')
    out = Path(export_dir) / f"{region.lower().replace(' ','_')}_sector_tiles_{day}.csv"
    df.to_csv(out, index=False)
    return str(out)

def detect_flips(df: pd.DataFrame, region: str, state_dir='data/state', alerts_dir='data/alerts'):
    Path(state_dir).mkdir(parents=True, exist_ok=True)
    Path(alerts_dir).mkdir(parents=True, exist_ok=True)
    state_path = Path(state_dir) / f"{region.lower().replace(' ','_')}_last.json"

    def _label(score):
        g = grade(score)
        if g.startswith('🟢'): return 'Strong'
        if g.startswith('🟡'): return 'Neutral'
        if g.startswith('🔴'): return 'Weak'
        return 'NA'
    ...
    for _, r in df.iterrows():
        sector = r['sector']
        label = _label(r['score'])
        current[sector] = label
        prev = last.get(sector)
        if prev and prev != label:
            flips.append({'sector': sector, 'from': prev, 'to': label})

    state_path.write_text(json.dumps(current, indent=2), encoding='utf-8')

    if flips:
        day = datetime.now().strftime('%Y%m%d_%H%M%S')
        alert_path = Path(alerts_dir) / f"{region.lower().replace(' ','_')}_flips_{day}.json"
        alert_path.write_text(json.dumps(flips, indent=2), encoding='utf-8')
        return flips, str(alert_path)
    return [], None
