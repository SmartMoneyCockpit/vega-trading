# US_Stock_Market.py
# USA Text Dashboard — EODHD-backed Scanner (EMA + Wedges + Long/Short/Momentum)
# + Economic Calendar (TV) + ALWAYS-ON Earnings table (EODHD) + TV widget add-on
...
TOKEN = _token()
if not TOKEN:
    st.warning("Set **EODHD_API_TOKEN** in your environment or `st.secrets` to enable search, scanner, and earnings table.")

# ──────────────────────────────────────────────────────────────────────────────
# EODHD helpers (Search + OHLCV + Earnings)
# ──────────────────────────────────────────────────────────────────────────────
@st.cache_data(ttl=300, show_spinner=False)
def eod_search_us(query: str, token: str) -> pd.DataFrame:
    """
    EODHD Symbol Search (US only).
    https://eodhd.com/api/search/{query}?api_token=...  -> returns list of {Code, Exchange, Name,...}
    """
    if not query.strip():
        return pd.DataFrame(columns=["Code","Exchange","Name"])
...
def fetch_ohlcv(symbol_eod: str, start: str, end: str, token: str) -> pd.DataFrame:
    """EODHD daily candles for USA: SYMBOL.US (or explicit .US provided)."""
...
# ──────────────────────────────────────────────────────────────────────────────
# Indicators, wedges, strategy tags
# ──────────────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────────────
# Scanner & Chart (USA + EODHD Search)
# ──────────────────────────────────────────────────────────────────────────────
...
with right:
    st.subheader("Local Scans (USA) — with EODHD Search")
    # Universe builder
    if "usa_universe" not in st.session_state:
        st.session_state["usa_universe"] = []

    search_q = st.text_input("Search EODHD (ticker or company name)", placeholder="e.g., MSFT or Microsoft")
    if st.button("🔎 Search"):
        if not TOKEN:
    ...
    if not df_s.empty:
        ...
        if st.button("➕ Add selected to Universe"):
            for p in picks:
    ...
    # Also allow manual paste list
    manual = st.text_area(
        "Or paste symbols (comma/newline)",
        value="",
        height=80
    )
    ...
    if universe:
        st.code(", ".join(universe), language="text")
    ...
    if r1.button("🚀 Run Scanner", use_container_width=True):
        if not universe:
            st.warning("Universe is empty. Use EODHD Search above or paste tickers, then run again.")
        else:
            with st.spinner("Scanning USA symbols…"):
                res = run_scan(universe, scan_kind)
    ...
    if not res.empty:
        ...
        st.info("Build your universe with EODHD Search above, then click **Run Scanner**.")

st.markdown("---")

# ──────────────────────────────────────────────────────────────────────────────
# Economic Calendar (TV) + ALWAYS-ON Earnings (EODHD) + TV widget add-on
# ──────────────────────────────────────────────────────────────────────────────
...
with col_right:
    ...
    if TOKEN:
        ...
        if not df_e.empty:
            # US-only view (if exchange present)
...
# Optional: TV earnings widget under the table (visual add-on)
with st.expander("📺 TradingView Earnings Widget (optional visual)"):
    components.html("""
    <div class="tradingview-widget-container">
      <div class="tradingview-widget-container__widget"></div>
      <script type="text/javascript"
